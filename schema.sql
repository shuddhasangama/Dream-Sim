-- Rebuilt 2026-09-10 from schema_postgres.sql, its exact parallel.
-- The two files differ only in identifier quoting, and SQLite accepts
-- the quoted spelling as well — it is case-insensitive, so "Ceremony"
-- and Ceremony name the same table. test_schema_postgres.py's parity
-- tests accept either, and verify the two files declare the same
