...
//
// road-fixes-clock-spec.md §6: Stats needs an edit entry point from both
// Dashboard and REACH. The "yours to change" field-grid below is exported
// so reach.js can render the exact same inline editor without a second
// copy of the field list/labels/multi-select handling — main.js's own
// dashboard just links to this screen instead of duplicating it.
...
// The "yours to change" editor alone — reused by REACH's inline editor,
// which only ever needs the open fields, never the held/verified section
// or the partner-disclosure history.
...
export function render(ctx) {
  ...
  return `<section class="intro"><span class="eyebrow">Your file · Stats</span><h1>Your stats</h1></section>
    ...
    <section class="card">
      ...
      ${editableFieldsForm(data, safe, 'stats-form')}
    </section>
    ...
    ${held.length ? `<form id="reverify-form"><section class="card">
      ...
      ${held.some((r) => r.why === 'verified') ? `<p class="warn">Sending a field for re-checking drops it out of "verified" until the recheck clears — REACH filters and match cards will show it as pending in the meantime.</p>
      <button class="btn-save secondary" type="submit">Send ticked for re-checking</button>` : ''}
